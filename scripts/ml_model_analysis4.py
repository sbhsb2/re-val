...
# ============================================================================
# MODEL 1B: DEMAND-BASED CLUSTERING
# ============================================================================

def cluster_neighborhoods_demand_based(cell_summary_csv, demand_csv):
    """
    DEMAND-BASED CLUSTERING MODEL
    
    What it does:
    - Groups cells by parcel demand patterns
    - Uses K-means clustering on demand features
    - Features: expected_parcels (demand volume)
    
    Why useful:
    - Identify high/low demand areas
    - Warehouse capacity planning
    - Resource allocation by demand zone
    
    Difference from Model 1:
    - Model 1 clusters on INFRASTRUCTURE (speed, circuity)
    - Model 1B clusters on DEMAND (parcel volume)
    - Both separate, can be combined
    
    LOGIC LINES: 440-620
    """
    print("="*70)
    print("MODEL 1B: DEMAND-BASED CLUSTERING")
    print("="*70)
    ...
    matched = (df['expected_parcels'] > 0).sum()
    print(f"Cells with demand: {matched}/{len(df)}")
    print(f"Total parcels: {df['expected_parcels'].sum():.0f}")
    
    # Features for demand-based clustering
    features = ['expected_parcels']
    
    # Optional: add density features
    if 'cell_population' in df.columns:
        features.append('cell_population')
    ...
    # Determine optimal k
    print("\nDetermining optimal clusters (demand-based)...")
    ...
    kmeans = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
    labels = kmeans.fit_predict(X_scaled)
    df_clean['demand_cluster'] = labels
    ...
    print(f"\n{'='*70}")
    print("DEMAND CLUSTER CHARACTERISTICS")
    print('='*70)
    
    for cluster_id in range(optimal_k):
        cluster_data = df_clean[df_clean['demand_cluster'] == cluster_id]
        
        avg_demand = cluster_data['expected_parcels'].mean()
        med_demand = cluster_data['expected_parcels'].median()
        max_demand = cluster_data['expected_parcels'].max()
        
        print(f"\nCluster {cluster_id} ({len(cluster_data)} cells)")
        print(f"  Demand: avg={avg_demand:.0f}, med={med_demand:.0f}, max={max_demand:.0f}")
        
        # Characterize by demand volume
        if avg_demand > df_clean['expected_parcels'].quantile(0.75):
            cluster_type = "Very High Demand"
        elif avg_demand > df_clean['expected_parcels'].median():
            cluster_type = "High Demand"
        elif avg_demand > df_clean['expected_parcels'].quantile(0.25):
            cluster_type = "Medium Demand"
        else:
    ...
    # Create visualizations
    create_demand_cluster_plots(df_clean, cluster_names, optimal_k)
    create_demand_cluster_map(df_clean, cluster_names, cluster_colors, optimal_k)
    
    # Save
    output_file = os.path.join(RESULTS_DIR, "demand_clustered_cells.csv")
    df_clean.to_csv(output_file, index=False)
    print(f"\nSaved: {output_file}")
...
def create_demand_cluster_plots(df_clustered, cluster_names, num_clusters):
    """Create visualizations of demand clusters."""
    print("\nGenerating demand cluster visualizations...")
    ...
    # 1. Demand by cluster
    ax = axes[0, 0]
    demand_by_cluster = [df_clustered[df_clustered['demand_cluster'] == i]['expected_parcels'].mean() 
                         for i in range(num_clusters)]
    ax.bar(range(num_clusters), demand_by_cluster, color=colors[:num_clusters])
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Average Parcels/Year')
    ax.set_title('Average Demand by Cluster')
    ax.grid(True, alpha=0.3)
    
    # 2. Cluster size
    ax = axes[0, 1]
    sizes = [len(df_clustered[df_clustered['demand_cluster'] == i]) for i in range(num_clusters)]
    ax.bar(range(num_clusters), sizes, color=colors[:num_clusters])
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Number of Cells')
    ax.set_title('Cluster Size Distribution')
    ax.grid(True, alpha=0.3)
    
    # 3. Demand distribution (boxplot)
    ax = axes[1, 0]
    demand_lists = [df_clustered[df_clustered['demand_cluster'] == i]['expected_parcels'].values 
                   for i in range(num_clusters)]
    bp = ax.boxplot(demand_lists, labels=range(num_clusters), patch_artist=True)
    for patch, color in zip(bp['boxes'], colors[:num_clusters]):
        patch.set_facecolor(color)
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Parcels/Year')
    ax.set_title('Demand Distribution by Cluster')
    ax.grid(True, alpha=0.3)
    
    # 4. Total demand per cluster
    ax = axes[1, 1]
    total_demand = [df_clustered[df_clustered['demand_cluster'] == i]['expected_parcels'].sum() 
                   for i in range(num_clusters)]
    ax.bar(range(num_clusters), total_demand, color=colors[:num_clusters])
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Total Parcels/Year')
    ax.set_title('Total Demand by Cluster')
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    plot_file = os.path.join(RESULTS_DIR, 'demand_cluster_statistics.png')
    plt.savefig(plot_file, dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"Saved demand cluster plot: {plot_file}")


def cluster_neighborhoods_demand_infrastructure(cell_summary_csv, demand_csv):
    """
    COMBINED CLUSTERING MODEL (BEST APPROACH)
    
    What it does:
    - Clusters on BOTH demand AND infrastructure
    - Features: expected_parcels + speed + circuity + population
    - Identifies: "High demand with poor infrastructure" = PRIORITY areas
    
    Why BEST:
    - Demand tells WHERE customers are
    - Infrastructure tells HOW GOOD delivery can be
    - Combined = optimal hub placement
    
    Output clusters:
    - Cluster 0: High Demand + Poor Infrastructure (URGENT)
    - Cluster 1: High Demand + Good Infrastructure (OPTIMAL)
    - Cluster 2: Low Demand + Poor Infrastructure (MONITOR)
    - Cluster 3: Low Demand + Good Infrastructure (EXPANSION)
    
    LOGIC LINES: 750-950
    """
    print("="*70)
    print("MODEL 1C: DEMAND + INFRASTRUCTURE CLUSTERING (COMBINED)")
    print("="*70)
    ...
    print(f"Matched {(df['expected_parcels'] > 0).sum()} cells with demand")
    
    # COMBINED FEATURES: Demand + Infrastructure
    features = ['expected_parcels']  # Demand
    ...
    print("\nDetermining optimal clusters (combined)...")
    ...
    kmeans = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
    labels = kmeans.fit_predict(X_scaled)
    df_clean['combined_cluster'] = labels
    ...
    print(f"\n{'='*70}")
    print("COMBINED CLUSTER CHARACTERISTICS")
    print("="*70)
    
    for cluster_id in range(optimal_k):
        cluster_data = df_clean[df_clean['combined_cluster'] == cluster_id]
        ...
        print(f"\nCluster {cluster_id} ({len(cluster_data)} cells)")
        print(f"  Demand: {avg_demand:.0f} parcels/year")
        print(f"  Speed: {avg_speed:.1f} km/h")
        print(f"  Circuity: {avg_cf:.2f}")
        
        # Categorize by demand + infrastructure
        demand_threshold = df_clean['expected_parcels'].median()
        speed_threshold = df_clean['avg_speed_auto'].median() if 'avg_speed_auto' in df_clean.columns else 30
        
        if avg_demand > demand_threshold and avg_speed < speed_threshold:
            cluster_type = "HIGH DEMAND + POOR INFRA (URGENT)"
        elif avg_demand > demand_threshold and avg_speed >= speed_threshold:
    ...
    # Visualizations
    create_combined_cluster_plots(df_clean, cluster_names, optimal_k)
    create_combined_cluster_map(df_clean, cluster_names, cluster_colors, optimal_k)
    
    # Save
    output_file = os.path.join(RESULTS_DIR, "combined_clustered_cells.csv")
    df_clean.to_csv(output_file, index=False)
    print(f"\nSaved: {output_file}")
...
def create_combined_cluster_plots(df_clustered, cluster_names, num_clusters):
    """Create visualizations of combined demand+infrastructure clusters."""
    print("\nGenerating combined cluster visualizations...")
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    colors = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00']
    
    # 1. Demand vs Speed scatter (colored by cluster)
    ax = axes[0, 0]
    for cluster_id in range(num_clusters):
        cluster_data = df_clustered[df_clustered['combined_cluster'] == cluster_id]
        ax.scatter(cluster_data['expected_parcels'], 
                  cluster_data['avg_speed_auto'],
                  label=f"C{cluster_id}",
                  color=colors[cluster_id],
                  s=80,
                  alpha=0.6)
    ax.set_xlabel('Demand (Parcels/Year)')
    ax.set_ylabel('Speed (km/h)')
    ax.set_title('Demand vs Infrastructure Quality')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # 2. Cluster distribution
    ax = axes[0, 1]
    sizes = [len(df_clustered[df_clustered['combined_cluster'] == i]) for i in range(num_clusters)]
    ax.bar(range(num_clusters), sizes, color=colors[:num_clusters])
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Number of Cells')
    ax.set_title('Cluster Size Distribution')
    ...
    # 3. Average metrics by cluster
    ax = axes[1, 0]
    cluster_ids = range(num_clusters)
    x_pos = np.arange(len(cluster_ids))
    width = 0.35
    
    avg_demand = [df_clustered[df_clustered['combined_cluster'] == i]['expected_parcels'].mean() 
                 for i in cluster_ids]
    avg_speed = [df_clustered[df_clustered['combined_cluster'] == i]['avg_speed_auto'].mean() 
                for i in cluster_ids]
    
    ax2 = ax.twinx()
    bars1 = ax.bar(x_pos - width/2, avg_demand, width, label='Demand', color='coral')
    bars2 = ax2.bar(x_pos + width/2, avg_speed, width, label='Speed', color='skyblue')
    
    ax.set_xlabel('Cluster')
    ax.set_ylabel('Demand (Parcels/Year)', color='coral')
    ax2.set_ylabel('Speed (km/h)', color='skyblue')
    ax.set_title('Demand and Speed by Cluster')
    ax.set_xticks(x_pos)
    ax.set_xticklabels(cluster_ids)
    ax.grid(True, alpha=0.3)
    
    # 4. Priority matrix (demand vs infrastructure)
    ax = axes[1, 1]
    ...
    for cluster_id in range(num_clusters):
        ...
        cluster_summary.append({
            'cluster': cluster_id,
            'demand': cluster_data['expected_parcels'].mean(),
            'speed': cluster_data['avg_speed_auto'].mean(),
            'size': len(cluster_data)
        })
    
    demand_median = df_clustered['expected_parcels'].median()
    speed_median = df_clustered['avg_speed_auto'].median()
    
    for item in cluster_summary:
        x = item['speed']
        y = item['demand']
        size = item['size'] * 3
        ax.scatter(x, y, s=size, color=colors[item['cluster']], alpha=0.6, 
                  edgecolors='black', linewidth=2)
        ax.annotate(f"C{item['cluster']}", (x, y), ha='center', va='center', fontweight='bold')
    
    ax.axhline(demand_median, color='red', linestyle='--', alpha=0.3, label='Demand median')
    ax.axvline(speed_median, color='blue', linestyle='--', alpha=0.3, label='Speed median')
    
    ax.set_xlabel('Infrastructure Quality (Speed km/h)')
    ax.set_ylabel('Demand (Parcels/Year)')
    ax.set_title('Priority Matrix: Demand vs Infrastructure')
    ax.grid(True, alpha=0.3)
    ax.legend()
    
    # Add quadrant labels
    ax.text(speed_median*1.2, demand_median*1.2, 'OPTIMAL', fontsize=10, alpha=0.5, ha='center')
    ax.text(speed_median*0.8, demand_median*1.2, 'URGENT', fontsize=10, alpha=0.5, ha='center', color='red')
    ax.text(speed_median*1.2, demand_median*0.8, 'EXPANSION', fontsize=10, alpha=0.5, ha='center')
    ax.text(speed_median*0.8, demand_median*0.8, 'MONITOR', fontsize=10, alpha=0.5, ha='center')
    
    plt.tight_layout()
    
    plot_file = os.path.join(RESULTS_DIR, 'combined_cluster_analysis.png')
    plt.savefig(plot_file, dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"Saved combined cluster plot: {plot_file}")


def create_combined_cluster_map(df_clustered, cluster_names, cluster_colors, num_clusters):
    """Create map showing combined clusters with grid squares."""
    print("Creating combined cluster map...")
    ...
    for _, row in df_clustered.iterrows():
        cluster_id = int(row['combined_cluster'])
        ...
        if 'geometry_4326' in row and pd.notna(row['geometry_4326']):
            ...
            if bounds:
                folium.Rectangle(
                    ...
                    fillColor=color,
                    fillOpacity=0.6,
                    popup=f"Cell: {row['cell_id']} | {cluster_name} | Demand: {row.get('expected_parcels', 'N/A'):.0f} | Speed: {row.get('avg_speed_auto', 'N/A'):.1f}"
                ).add_to(m)
        else:
            cell_size_deg = 0.005
            folium.Rectangle(
                ...
                fillColor=color,
                fillOpacity=0.6,
                popup=f"Cell: {row['cell_id']} | {cluster_name}"
            ).add_to(m)
    ...
    <p style="margin: 0 0 10px 0; font-weight: bold; font-size: 14px;">
        DEMAND + INFRASTRUCTURE CLUSTERS
    </p>
    ...
    for cluster_id in range(num_clusters):
        ...
        legend_html += f'''
    <div style="margin: 8px 0; padding: 10px; background-color: {color}; 
                border: 2px solid black; border-radius: 3px; opacity: 0.8;">
        <p style="margin: 0; font-weight: bold; color: white; text-shadow: 1px 1px 1px black; font-size: 11px;">
            Cluster {cluster_id}
        </p>
    ...
    <p style="margin: 5px 0; font-size: 10px; color: #666;">
        OPTIMAL = High demand + Good infrastructure
        URGENT = High demand + Poor infrastructure
        EXPANSION = Low demand + Good infrastructure
        MONITOR = Low demand + Poor infrastructure
    </p>
    </div>
    '''
    ...
    map_file = os.path.join(RESULTS_DIR, f"combined_cluster_map_{timestamp}.html")
    m.save(map_file)
    print(f"Saved combined cluster map: {map_file}")


def create_demand_cluster_map(df_clustered, cluster_names, cluster_colors, num_clusters):
    """Create map showing demand clusters with grid squares."""
    print("Creating demand cluster map...")
    ...
    # Draw grid squares colored by demand cluster
    for _, row in df_clustered.iterrows():
        cluster_id = int(row['demand_cluster'])
        color = cluster_colors.get(cluster_id, '#cccccc')
        cluster_name = cluster_names.get(cluster_id, f"Cluster {cluster_id}")
        ...
        if 'geometry_4326' in row and pd.notna(row['geometry_4326']):
            ...
            if bounds:
                folium.Rectangle(
                    ...
                    fillColor=color,
                    fillOpacity=0.6,
                    popup=f"Cell: {row['cell_id']} | Demand Cluster {cluster_id}: {cluster_name} | Parcels: {row.get('expected_parcels', 'N/A'):.0f}"
                ).add_to(m)
        else:
            cell_size_deg = 0.005
    ...
    <hr style="margin: 8px 0;">
    <p style="margin: 5px 0; font-size: 10px; color: #666;">
        Color shows demand cluster membership
    </p>
    </div>
    '''
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    map_file = os.path.join(RESULTS_DIR, f"demand_cluster_map_{timestamp}.html")
    m.save(map_file)
    print(f"Saved demand cluster map: {map_file}")
...
def main():
    """Run all ML models with comprehensive visualizations."""
    print("="*70)
    print("MACHINE LEARNING MODELS FOR LAST-MILE LOGISTICS")
    ...
    if os.path.exists(demand_data):
        print("\n" + "="*70)
        print("PART 2: DEMAND-AWARE CLUSTERING & OPTIMIZATION")
        print("="*70)
        
        # Model 1B: Demand-only clustering
        print("\nModel 1B: Demand-Based Clustering")
        df_demand_clusters = cluster_neighborhoods_demand_based(cell_summary, demand_data)
        
        # Model 1C: COMBINED Demand + Infrastructure (BEST)
        print("\nModel 1C: Combined Demand + Infrastructure Clustering")
        df_combined_clusters = cluster_neighborhoods_demand_infrastructure(cell_summary, demand_data)
        
        # Models 5, 6, 7: Optimization & Prediction
        print("\nModels 5-7: Hub Optimization & Demand Prediction")
        hubs_demand = optimize_hubs_with_demand(cell_summary, demand_data, num_hubs=5, coverage_km=3.0)
        demand_model, demand_features = train_demand_predictor(demand_data, cell_summary)
        df_demand_infra_clusters = cluster_by_demand_and_infrastructure(cell_summary, demand_data)
    else:
        print(f"\nDemand data not found: {demand_data}")
        print("   Skipping Models 1B-7")
    ...
    print("\nCSV Files Generated:")
    print("  - clustered_cells.csv (Model 1: Infrastructure clustering)")
    print("  - demand_clustered_cells.csv (Model 1B: Demand clustering)")
    print("  - combined_clustered_cells.csv (Model 1C: Demand + Infrastructure)")
    print("  - optimal_hub_locations.csv (Model 3)")
    if os.path.exists(demand_data):
        print("  - demand_aware_hubs.csv (Model 5)")
        print("  - demand_infra_clusters.csv (Model 7)")
    
    print("\nVisualization Graphics (PNG):")
    print("  - elbow_method_analysis.png (Optimal cluster selection)")
    print("  - cluster_statistics.png (Infrastructure cluster characteristics)")
    print("  - demand_cluster_statistics.png (Demand cluster characteristics)")
    print("  - combined_cluster_analysis.png (Priority matrix: Demand vs Infrastructure)")
    print("  - comprehensive_results_analysis.png (All key metrics)")
    
    print("\nInteractive Maps (HTML):")
    print("  - cluster_map_grid_*.html (Infrastructure clusters)")
    print("  - demand_cluster_map_*.html (Demand clusters)")
    print("  - combined_cluster_map_*.html (Demand + Infrastructure)")
